...
def visalize_rots(ref, target, rot):
    ...
    for i in range(0, L, int(L/10)):
        ...
        angles.append(angle)
    print(f"mean r:{np.mean(np.linalg.norm(ref, axis=0))}, mean t: {np.mean(np.linalg.norm(target, axis=0))}, rmse angle:{np.linalg.norm(angles)/len(angles)}")
...
def calibrate_translation(samples):
    deltas = []
    ...
    A = coefficients
    B = constants
    costumOutlierRemoval = CostumOutlierRemoval()
    ...
    print(f"Calibrated translation x={transcm[0]:.2f} y={transcm[1]:.2f} z={transcm[2]:.2f}")
